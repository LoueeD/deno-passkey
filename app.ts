...
function getCookieValueByName(req: Request, name: string) {
  const cookies = req.headers.get('cookie');
  if (!cookies) throw new Error('No cookies found');
  // Get name followed by anything except a semicolon
  const cookiestring = RegExp(name+"=[^;]+").exec(cookies);
...
const handler: Deno.ServeHandler = async (req) => {
  ...
  switch (url.pathname) {
    ...
    case '/login/verify': {
      ...
      if (verification.verified) {
        ...
        return new Response(JSON.stringify(verification), {
          headers: {
            'content-type': 'application/json',
            'set-cookie': `deno_passkey_token=${await generateJWT(userId)}; HttpOnly; Secure; SameSite=Strict; Path=/; Expires=${new Date(Date.now() + 600_000).toUTCString()}`,
          },
        });
      }
    ...
    case '/private': {
      ...
      const token = getCookieValueByName(req, 'deno_passkey_token');
